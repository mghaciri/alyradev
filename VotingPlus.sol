...
contract VotingPlus is Ownable {
    ...
    {
        // L'adminisytrateur a aussi le droit de voter
        setWhitelist(initialOwner);
    }
    ...
    modifier isWhitelisted() {
        ...
        require(!whitelist[msg.sender].hasVoted,unicode"Vous n'êtes pas autorisé à revoter :)");
    ...
    function isExists(string memory _proposal) internal view returns (bool) {
      // Rechercher si la proposition existe dejà dans le tableau de propositions
    ...
    function submitProposal(string memory _proposal) public isWhitelisted {
        ...
        // Si la proposition n'existe pas, l'ajouter
        if (!isExists(_proposal)) {
